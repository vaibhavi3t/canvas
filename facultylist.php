...
<table width="717" border="1">
  <tr style="background: #333; border: 1px solid #331;color:#fff; font-weight: bold;">
    <td><strong>&nbsp;Faculty List </strong></td>
...
$result = mysql_query("SELECT * FROM members WHERE confirmation = '0' AND type = 'Faculty' ORDER BY member_id DESC LIMIT 5");
while($row = mysql_fetch_array($result))
  {
  echo '<div align="left" class="style46" style="padding-left: 20px; ">';
  echo '&nbsp;';
 echo '<a href=facultylist.php?id=' . $row["member_id"] . '>';
...
while($row = mysql_fetch_array($result))
  { ?>
   <form id="basic-modal" action="facultylist.php?id=<?php echo $_GET['id'] ?>" method="post">
    <?php 
  echo '<br />';
  ...
  echo '<span style="color: #ffff00;">Gender : </span>' . $row['gender'] . "<br />";
  echo '</div>';
  echo '<br />';		
?>
...
www.Canvas.pcriot.com

Have fun..</textarea>
<br />
<br />
<input type="submit" name="send" value="Send" />
<a href="facultylist.php"><input type="submit" name="cancel" value="Cancel" /></a></label>
</label>
</form>';
$con = '1';
...
echo "Member deleted.";
echo "<a href='facultylist.php'><input type='submit' value='OK' name='ok' /></a>";
}
?>
...
<textarea name="content" cols="45" rows="6" id="textarea">
Sorry!!
You can not create an account at Canvas..
